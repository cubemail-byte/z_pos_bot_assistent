...
## Таблица `message_entities` — извлечённые сущности (КЕ)

Таблица содержит **результат извлечения фактов из сообщений**.

### Назначение
- фиксация упоминаний КЕ и реквизитов
- поддержка enrichment
- основа для аналитики повторяемости проблем
...
|----|----|----|
| id | INTEGER PK | Уникальный идентификатор |
| message_id | INTEGER | Ссылка на messages.id |
| entity_type | TEXT | Тип сущности (`azs`, `workplace`, `tid`, `ip`, `sd_ticket`, …) |
| entity_value | TEXT | Нормализованное значение |
| entity_raw | TEXT | Сырой фрагмент текста |
| confidence | REAL | Уверенность |
| extractor | TEXT | Источник (`regex:v1`, `directory:v1`) |
| created_at_utc | TEXT | Время извлечения |

### Особенности
- одна сущность = одна строка
- для одного сообщения допустимо несколько `workplace`
- `tid/ip` добавляются через enrichment
